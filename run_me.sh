...
python download.py --bert --model ${MODEL_SIZE} --ckpt ${MODEL_CHECKPOINT} --download_dir ${DOWNLOAD_DIR}  # pre-trained model
...
# sampling (model S version)

SAMPLING_EMBEDDING=512
SAMPLING_HEADS=8
SAMPLING_LAYERS=24

CUDA_VISIBLE_DEVICES=0 python src/run.py --bert --n_vocab 256 --sample --n_embd ${SAMPLING_EMBEDDING} --n_head ${SAMPLING_HEADS} --n_layer ${SAMPLING_LAYERS} -ckpt_path ${DOWNLOAD_DIR} --color_cluster_path ${DOWNLOAD_DIR} --n_gpu 1
